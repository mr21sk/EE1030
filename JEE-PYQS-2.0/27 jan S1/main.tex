...
\begin{enumerate}
\item If f(x) = $ \myvec{\cos{x} & -\sin{x} & 0 \\ \sin{x} & \cos{x} & 0 \\ 0 & 0 & 1 } $ then \\ Statement 1 : f(-x) is inverse of f(x) . \\ Statement 2 :f(x + y) = f(x)f(y) . 
...
\end{multicols}
\end{enumerate}
\item Let $\vec{a}=\hat{i}+2\hat{j}+\hat{k}$ , $\vec{b}=3(\hat{i}-\hat{j}+\hat{k})$ $\cdot$ Let $\vec{c}$ be the vector such that $\vec{a}$x$\vec{c}$=$\vec{b}$ and $\vec{a}$$\cdot$$\vec{c}=3$ . Let $\vec{a}$$\cdot$$\brak{\brak{\vec{b}\text{x}\vec{c}}-\vec{b}-\vec{c}}$ is equal to : 
\hfill{\brak{\text{Jan 2024}}}
\begin{enumerate}   
\begin{multicols}{4}
\item $24$
