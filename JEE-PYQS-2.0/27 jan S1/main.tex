...
\begin{enumerate}
\item If f\brak{x} = $ \myvec{\cos{x} & -\sin{x} & 0 \\ \sin{x} & \cos{x} & 0 \\ 0 & 0 & 1 } $ then \\ Statement 1 : f\brak{-x} is inverse of f\brak{x} . \\ Statement 2 :f\brak{x + y} = f\brak{x}f\brak{y}. \\
\begin{enumerate}
\item Both are true 
\item Both are false 
...
\end{enumerate} \\ 
\item If $S=\cbrak{z\in\mathbb{C}:\abs{z-i}=\abs{z-1}=\abs{z+i}}$ , then the $n\brak{S}$ is : \\
\begin{enumerate}
\begin{multicols}{4}
\item $ 2 $
\item $ 3 $
...
\end{multicols}
\end{enumerate}
\item Let $a=\hat{i}+2\hat{j}+\hat{k}$ , $b=3(\hat{i}-\hat{j}+\hat{k})$ $\cdot$ Let $c$ be the vector such that $a \times c=b$ and $a$$\cdot$$c=3$ . Let $a$$\cdot$$\brak{\brak{b \times c}-b-c}$ is equal to : \\
\begin{enumerate}   
\begin{multicols}{4}
\item $24$
\item $36$ 
